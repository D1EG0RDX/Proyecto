...
def usuarios():
    if request.method == 'POST':
        ...
        cursor = conn.cursor()
        cursor.execute('insert into r_empresa (correo,contrasena) values (%s, %s)',(aux_Correo, aux_Contrasena))
        conn.commit()
...
def iniciar_sesion():
    if request.method == 'POST':
        ...
        conn = pymysql.connect(host='localhost', user='root', passwd='', db='agenda', port=3306)
        cursor = conn.cursor()
        cursor.execute('select * from r_empresa where correo=%s and contrasena=%s', (aux_Correo, aux_Contrasena))
